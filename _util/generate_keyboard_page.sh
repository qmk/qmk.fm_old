...
while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    if [ -e compiled/${keyboard}_default.hex ]; then
        ...
        echo -n "[<i class='fa fa-download' aria-hidden='true'></i>](http://qmk.fm/compiled/${keyboard}_default.hex \"$line's latest default QMK firmware\")" >> _pages/keyboards.md
        echo -n "[<span class='fa-stack fa-lg'><i class='fa fa-circle fa-stack-1x'></i><i class='fa fa-inverse fa-stack-1x psi-icon'>&Psi;</i></span>](qmk:http://qmk.fm/compiled/${keyboard}_default.hex \"Open in QMK Toolbox\")" >> _pages/keyboards.md
